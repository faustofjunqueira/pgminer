...
FANN_EXTERNAL struct fann *FANN_API fann_create_standard(unsigned int num_layers, ...)
{
	...
	va_list layer_sizes;
	int i;
	unsigned int *layers = (unsigned int *) pgm_calloc(num_layers, sizeof(unsigned int));
															 ...
															 const unsigned int *layers)
{
	...
	for(layer_it = ann->first_layer + 1; layer_it != ann->last_layer; layer_it++)
	{
		...
		for(i = 0; i != num_neurons_out; i++)
		{
			...
			layer_it->first_neuron[i].activation_function = NN_SIGMOID_STEPWISE;
#ifdef FIXEDFANN
	...
	else
	{
		...
		/* All the connections are cleared by pgm_calloc, because we want to
...
FANN_EXTERNAL fann_type *FANN_API fann_run(struct fann * ann, fann_type * input)
{
	...
	for(i = 0; i != num_input; i++)
	{
		...
		//elog(INFO, "first_neuron[%d].value = %lf", i, input[i]);
	}
	...
	for(layer_it = ann->first_layer + 1; layer_it != last_layer; layer_it++)
	{
		...
		for(neuron_it = layer_it->first_neuron; neuron_it != last_neuron; neuron_it++)
		{
			...
			if(ann->connection_rate >= 1)
			{
				...
				for(; i != num_connections; i += 4)
				{
					neuron_sum +=
						...
						//Debug
						/*elog( INFO, "%d: %lf - %lf\n", i+0, weights[i + 0], neurons[i + 0].value );
						elog( INFO, "%d: %lf - %lf\n", i+1, weights[i + 1], neurons[i + 1].value );
						elog( INFO, "%d: %lf - %lf\n", i+2, weights[i + 2], neurons[i + 2].value );
						elog( INFO, "%d: %lf - %lf\n", i+3, weights[i + 3], neurons[i + 3].value );*/
			...
			else
			{
				...
				for(; i != num_connections; i += 4)
				{
					neuron_sum +=
						...
						//Debug
						/*
						elog( INFO, "%d: %lf - %lf\n", i+0, weights[i + 0], neuron_pointers[i + 0]->value );
						elog( INFO, "%d: %lf - %lf\n", i+1, weights[i + 1], neuron_pointers[i + 1]->value );
						elog( INFO, "%d: %lf - %lf\n", i+2, weights[i + 2], neuron_pointers[i + 2]->value );
						elog( INFO, "%d: %lf - %lf\n", i+3, weights[i + 3], neuron_pointers[i + 3]->value ); */
			...
			nn_activation_switch(activation_function, neuron_sum, neuron_it->value);

			//elog (INFO, "neuron_it f %d neuron_it l %d neuron_sum %lf, neuron_it->value %lf %d",neuron_it->first_con,neuron_it->last_con,(double) neuron_sum, (double) neuron_it->value,activation_function);

#endif
		}
	}
	//elog(INFO, "Activation function: %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d ",NN_LINEAR,NN_LINEAR_PIECE,NN_LINEAR_PIECE_SYMMETRIC,NN_SIGMOID,NN_SIGMOID_SYMMETRIC,NN_SIGMOID_SYMMETRIC_STEPWISE,NN_SIGMOID_STEPWISE,NN_THRESHOLD,NN_THRESHOLD_SYMMETRIC,NN_GAUSSIAN,NN_GAUSSIAN_SYMMETRIC,NN_ELLIOT,NN_ELLIOT_SYMMETRIC,NN_SIN_SYMMETRIC,NN_COS_SYMMETRIC,NN_SIN,NN_COS,NN_GAUSSIAN_STEPWISE);
	/* set the output */
	output = ann->output;
...
FANN_EXTERNAL struct fann* FANN_API fann_copy(struct fann* orig)
{
    ...
    if (orig->errstr)
    {
        copy->errstr = (char *) pgm_malloc(FANN_ERRSTR_MAX);
        if (copy->errstr == NULL)
        {
            fann_destroy(copy);
            return NULL;
   ...
   /* copy cascade activation functions */
    ...
    copy->cascade_activation_functions = (enum nn_ActivationFuncEnum *)realloc(copy->cascade_activation_functions,
        copy->cascade_activation_functions_count * sizeof(enum nn_ActivationFuncEnum));
    ...
    else
    {
        copy->cascade_candidate_scores =
            (fann_type *) pgm_malloc(fann_get_cascade_num_candidates(copy) * sizeof(fann_type));
 ...
 /* copy the connections */
    ...
    {
        copy->train_slopes = (fann_type *) pgm_malloc(copy->total_connections_allocated * sizeof(fann_type));
    ...
    if (orig->prev_steps)
    {
        copy->prev_steps = (fann_type *) pgm_malloc(copy->total_connections_allocated * sizeof(fann_type));
    ...
    {
        copy->prev_train_slopes = (fann_type *) pgm_malloc(copy->total_connections_allocated * sizeof(fann_type));
    ...
    {
        copy->prev_weights_deltas = (fann_type *) pgm_malloc(copy->total_connections_allocated * sizeof(fann_type));
...
FANN_EXTERNAL void FANN_API fann_print_connections(struct fann *ann)
{
	...
	neurons = (char *) pgm_malloc(num_neurons + 1);
	if(neurons == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		return;
		...
		for(neuron_it = layer_it->first_neuron; neuron_it != layer_it->last_neuron; neuron_it++)
		{
			...
			for(i = neuron_it->first_con; i < neuron_it->last_con; i++)
			{
				if(ann->weights[i] < 0)
				{
					...
					if(value < -25)
						value = -25;
					neuronsfann_set_weight[ann->connections[i] - ann->first_layer->first_neuron] = (char)('a' - value);
				}
				else
				{
#ifdef FIXEDFANN
	...
	for(layer_it = ann->first_layer + 1; layer_it != ann->last_layer; layer_it++)
	{
		...
		for(neuron_it = layer_it->first_neuron; neuron_it != last_neuron; neuron_it++)
		{
				...
				num_connect++)
			{
				...
				{
#ifdef FIXEDFANNfann_set_weight
...
struct fann *fann_allocate_structure(unsigned int num_layers)
{
	...
	ann = (struct fann *) pgm_malloc(sizeof(struct fann));
	if(ann == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		return NULL;
	...
	ann->cascade_activation_functions =
		(enum nn_ActivationFuncEnum *)pgm_calloc(ann->cascade_activation_functions_count,
							   sizeof(enum nn_ActivationFuncEnum));
	...
	ann->cascade_activation_functions[0] = NN_SIGMOID;
	ann->cascade_activation_functions[1] = NN_SIGMOID_SYMMETRIC;
	ann->cascade_activation_functions[2] = NN_GAUSSIAN;
	ann->cascade_activation_functions[3] = NN_GAUSSIAN_SYMMETRIC;
	ann->cascade_activation_functions[4] = NN_ELLIOT;
	ann->cascade_activation_functions[5] = NN_ELLIOT_SYMMETRIC;
	ann->cascade_activation_functions[6] = NN_SIN_SYMMETRIC;
	ann->cascade_activation_functions[7] = NN_COS_SYMMETRIC;
	ann->cascade_activation_functions[8] = NN_SIN;
	ann->cascade_activation_functions[9] = NN_COS;

	ann->cascade_activation_steepnesses_count = 4;
	ann->cascade_activation_steepnesses =
		(fann_type *)pgm_calloc(ann->cascade_activation_steepnesses_count,
							   sizeof(fann_type));
	...
	ann->first_layer = (struct fann_layer *) pgm_calloc(num_layers, sizeof(struct fann_layer));
...
#define SCALE_ALLOCATE( what, where, default_value )		    			\
		ann->what##_##where = (float *)pgm_calloc(								\
			ann->num_##where##put,											\
			sizeof( float )													\
			);																\
...
void fann_allocate_neurons(struct fann *ann)
{
	...
	/* all the neurons is allocated in one long array (pgm_calloc clears mem) */
	neurons = (struct fann_neuron *) pgm_calloc(ann->total_neurons, sizeof(struct fann_neuron));
	...
	ann->output = (fann_type *) pgm_calloc(num_neurons, sizeof(fann_type));
	if(ann->output == NULL)
	{
		fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
		return;
...
{
	ann->weights = (fann_type *) pgm_calloc(ann->total_connections, sizeof(fann_type));
	if(ann->weights == NULL)
	{
		fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
		return;
	 ...
	 */
	ann->connections =
		(struct fann_neuron **) pgm_calloc(ann->total_connections_allocated,
