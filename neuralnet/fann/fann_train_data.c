...
/* Need Prototype for PostgreSQL 9.1 */
											   ...
											   float desired_error)
{
	...
	{
		elog(INFO,"Max epochs %8d. Desired error: %.10f.\n", max_epochs, desired_error);
	}
	...
	for(i = 1; i <= max_epochs; i++)
	{
			...
			desired_error_reached == 0))
		{
			...
			{
				elog(INFO,"Epochs     %8d. Current error: %.10f. Bit fail %d.\n", i, error,
					   ann->num_bit_fail);
			}
																	 ...
																	 struct fann_train_data *data2)
{
	...
	struct fann_train_data *dest =
		(struct fann_train_data *) pgm_malloc(sizeof(struct fann_train_data));
  ...
  */
	...
	dest->input = (fann_type **) pgm_calloc(dest->num_data, sizeof(fann_type *));
	if(dest->input == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
	...
	dest->output = (fann_type **) pgm_calloc(dest->num_data, sizeof(fann_type *));
	if(dest->output == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(dest);
	...
	data_input = (fann_type *) pgm_calloc(dest->num_input * dest->num_data, sizeof(fann_type));
	if(data_input == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(dest);
	...
	data_output = (fann_type *) pgm_calloc(dest->num_output * dest->num_data, sizeof(fann_type));
	if(data_output == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(dest);
...
FANN_EXTERNAL struct fann_train_data * FANN_API fann_create_train(unsigned int num_data, unsigned int num_input, unsigned int num_output)
{
	...
	data->input = (fann_type **) pgm_calloc(num_data, sizeof(fann_type *));
	if(data->input == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
	...
	data->output = (fann_type **) pgm_calloc(num_data, sizeof(fann_type *));
	if(data->output == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
	...
	data_input = (fann_type *) pgm_calloc(num_input * num_data, sizeof(fann_type));
	if(data_input == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
	...
	data_output = (fann_type *) pgm_calloc(num_output * num_data, sizeof(fann_type));
	if(data_output == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
