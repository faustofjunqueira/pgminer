...
struct fann *fann_create_from_fd(FILE * conf, const char *configuration_file)
{
	...
	read_version = (char *) pgm_calloc(strlen(FANN_CONF_VERSION "\n"), 1);
	...
	ann->cascade_activation_functions =
		(enum nn_ActivationFuncEnum *)realloc(ann->cascade_activation_functions,
		ann->cascade_activation_functions_count * sizeof(enum nn_ActivationFuncEnum));
...
#undef SCALE_LOAD
	...
	for(neuron_it = ann->first_layer->first_neuron; neuron_it != last_neuron; neuron_it++)
	{
		...
		neuron_it->activation_function = (enum nn_ActivationFuncEnum)tmpVal;
...
struct fann *fann_create_from_fd_1_1(FILE * conf, const char *configuration_file)
{
	...
	fann_set_activation_function_hidden(ann, (enum nn_ActivationFuncEnum)activation_function_hidden);
	fann_set_activation_function_output(ann, (enum nn_ActivationFuncEnum)activation_function_output);
