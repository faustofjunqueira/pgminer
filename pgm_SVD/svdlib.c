...
DMat svdNewDMat(int rows, int cols) {
  int i;
  DMat D = (DMat) palloc0(sizeof(struct dmat));
  if (!D) {perror("svdNewDMat"); return NULL;}
  D->rows = rows;
  D->cols = cols;

  D->value = (double **) palloc(rows * sizeof(double *));
  if (!D->value) {SAFE_FREE(D); return NULL;}

  D->value[0] = (double *) palloc0(rows * cols * sizeof(double));
...
SMat svdNewSMat(int rows, int cols, int vals) {
  SMat S = (SMat) palloc( sizeof(struct smat));
...
SVDRec svdNewSVDRec(void) {
  SVDRec R = (SVDRec) palloc( sizeof(struct svdrec));
