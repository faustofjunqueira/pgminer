...
/*  ---------  */
{
    ...
    Raw = (Tree *) pgm_calloc(1, sizeof(Tree));
    Pruned = (Tree *) pgm_calloc(1, sizeof(Tree));
...
/*    --------  */
{
    ...
    TargetClassFreq = (ItemNo *) pgm_calloc(MaxClass+1, sizeof(ItemNo));

    Raw    = (Tree *) pgm_calloc(TRIALS, sizeof(Tree));
    Pruned = (Tree *) pgm_calloc(TRIALS, sizeof(Tree));
    ...
    ItemNo Size;
{
    ...
    ClassFreq = (ItemNo *) pgm_calloc(MaxClass+1, sizeof(ItemNo));
...
/*************************************************************************/
    ...
    Evaluate(CMInfo, Saved, resultado)
/*  --------  */
    Boolean CMInfo;
    short Saved;
    char* resultado;
{
    ...
    if ( CMInfo )
    {
	ConfusionMat = (ItemNo *) pgm_calloc((MaxClass+1)*(MaxClass+1), sizeof(ItemNo));
    }
    ...
    sprintf(buffer, "\t%4d  %3d(%4.1f%%)   %4d  %3d(%4.1f%%)    (%4.1f%%)\n",
           TreeSize(Raw[t]), RawErrors, 100.0*RawErrors / (MaxItem+1.0),
           TreeSize(Pruned[t]), PrunedErrors, 100.0*PrunedErrors / (MaxItem+1.0),
           100 * Pruned[t]->Errors / Pruned[t]->Items);
    }
	...
	pgm_free(ConfusionMat);
    }


    strcat(resultado, "\t Before Pruning           After Pruning\n");
    strcat(resultado, "\t----------------   ---------------------------\n");
    strcat(resultado, "\tSize      Errors   Size      Errors   Estimate\n\n");
