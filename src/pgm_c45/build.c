...
/*  ------------------  */
{
    ...
    Tested	= (char *) pgm_calloc(MaxAtt+1, sizeof(char));

    Gain	= (float *) pgm_calloc(MaxAtt+1, sizeof(float));
    Info	= (float *) pgm_calloc(MaxAtt+1, sizeof(float));
    Bar		= (float *) pgm_calloc(MaxAtt+1, sizeof(float));

    Subset = (Set **) pgm_calloc(MaxAtt+1, sizeof(Set *));
    ForEach(a, 0, MaxAtt)
    {
	if ( MaxAttVal[a] )
	{
	    Subset[a]  = (Set *) pgm_calloc(MaxDiscrVal+1, sizeof(Set));
	    ForEach(v, 0, MaxAttVal[a])
	    {
		Subset[a][v] = (Set) pgm_malloc((MaxAttVal[a]>>3) + 1);
	    }
	}
    }
    Subsets = (short *) pgm_calloc(MaxAtt+1, sizeof(short));

    SplitGain = (float *) pgm_calloc(MaxItem+1, sizeof(float));
    SplitInfo = (float *) calloc(MaxItem+1, sizeof(float));

    Weight = (ItemCount *) pgm_calloc(MaxItem+1, sizeof(ItemCount));

    Freq  = (ItemCount **) pgm_calloc(MaxDiscrVal+1, sizeof(ItemCount *));
    ForEach(v, 0, MaxDiscrVal)
    {
	Freq[v]  = (ItemCount *) pgm_calloc(MaxClass+1, sizeof(ItemCount));
    }

    ValFreq = (ItemCount *) pgm_calloc(MaxDiscrVal+1, sizeof(ItemCount));
    ClassFreq = (ItemCount *) pgm_calloc(MaxClass+1, sizeof(ItemCount));

    Slice1 = (ItemCount *) pgm_calloc(MaxClass+2, sizeof(ItemCount));
    Slice2 = (ItemCount *) pgm_calloc(MaxClass+2, sizeof(ItemCount));

    UnknownRate = (float *) pgm_calloc(MaxAtt+1, sizeof(float));
