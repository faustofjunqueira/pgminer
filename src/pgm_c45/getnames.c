...
int sCtr = 0;
int str_getc(char *str) { return str[sCtr++]; }

#define  Space(s)	(s == ' ' || s == '\n' || s == '\t' || s=='(' || s==')' || s == '\"')
#define  SkipComment	while ( ( c = str_getc(f) ) != ')'  )
...
/*************************************************************************/
/*									 */
/*  Read a name from string s_in into string s, setting Delimiter.		 */
/*									 */
/*  - Embedded periods are permitted, but periods followed by space	 */
/*    characters act as delimiters.					 */
/*  - Embedded spaces are permitted, but multiple spaces are replaced	 */
...
/*
    char arquivo[1024] = "0, 1 ) \
			 ...
			 (attribute#4:1, 2, 3 \") \
                         ...
                         (attribute#6:1, 2 \")";
  */
     //char arquivo[1024] = "0, 1)(attribute#1:1,        2, 3\")(attribute#2:1, 2, 3\")(attribute#3:1, 2\")(attribute#4:1, 2, 3\")(attribute#5:1, 2, 3, 4\")(attribute#6:1, 2\")";
   ...
   // if ( ! ( Nf = fopen(Fn, "r") ) ) Error(0, Fn, "");
    ...
    ClassName = (String *) pgm_calloc(ClassCeiling, sizeof(String));
    MaxClass = -1;
    do
    {
	sReadName(arquivo, Buffer);

	if ( ++MaxClass >= ClassCeiling)
	{
	    ClassCeiling += 100;
	    ClassName = (String *) pgm_realloc(ClassName, ClassCeiling*sizeof(String));
	}
    ...
    AttName = (String *) pgm_calloc(AttCeiling, sizeof(String));
    MaxAttVal = (DiscrValue *) pgm_calloc(AttCeiling, sizeof(DiscrValue));
    AttValName = (String **) pgm_calloc(AttCeiling, sizeof(String *));
    SpecialStatus = (char *) pgm_malloc(AttCeiling);

    MaxAtt = -1;
    while ( sReadName(arquivo, Buffer) )
    {
	...
	{
	    AttCeiling += 100;
	    AttName = (String *) pgm_realloc(AttName, AttCeiling*sizeof(String));
	    MaxAttVal = (DiscrValue *) pgm_realloc(MaxAttVal, AttCeiling*sizeof(DiscrValue));
	    AttValName = (String **) pgm_realloc(AttValName, AttCeiling*sizeof(String *));
	    SpecialStatus = (char *) pgm_realloc(SpecialStatus, AttCeiling);
	}
	...
	MaxAttVal[MaxAtt] = 0;
	ValCeiling = 100;
	AttValName[MaxAtt] = (String *) pgm_calloc(ValCeiling, sizeof(String));



	do
	{
	    if ( ! ( sReadName(arquivo, Buffer) ) ) Error(2, AttName[MaxAtt], "");

	    if ( ++MaxAttVal[MaxAtt] >= ValCeiling )
	    {
		ValCeiling += 100;
		AttValName[MaxAtt] =
		    (String *) pgm_realloc(AttValName[MaxAtt], ValCeiling*sizeof(String));
	...
	if ( MaxAttVal[MaxAtt] == 1 )
	{
		...
		AttValName[MaxAtt] =
		    (String *) pgm_realloc(AttValName[MaxAtt], (v+2)*sizeof(String));
   ...
   // printf("aaa: %d | %d | %d",MaxAtt, MaxClass, MaxDiscrVal);
    ...
    String x;
{
    ...
    s = (char *) pgm_calloc(strlen(x)+1, sizeof(char));
    strcpy(s, x);
    return s;
}
