								  ...
								  fann_type steepness, fann_type value, fann_type sum)
{
	switch (activation_function)
	{
		...
		case NN_LINEAR_PIECE_SYMMETRIC:
			return (fann_type) nn_linear_derive(steepness, value);
		case NN_SIGMOID:
		case NN_SIGMOID_STEPWISE:
			value = fann_clip(value, 0.01f, 0.99f);
			return (fann_type) nn_sigmoid_derive(steepness, value);
		case NN_SIGMOID_SYMMETRIC:
		case NN_SIGMOID_SYMMETRIC_STEPWISE:
			value = fann_clip(value, -0.98f, 0.98f);
			return (fann_type) nn_sigmoid_symmetric_derive(steepness, value);
		case NN_GAUSSIAN:
			/* value = fann_clip(value, 0.01f, 0.99f); */
			return (fann_type) nn_gaussian_derive(steepness, value, sum);
		case NN_GAUSSIAN_SYMMETRIC:
			/* value = fann_clip(value, -0.98f, 0.98f); */
			return (fann_type) nn_gaussian_symmetric_derive(steepness, value, sum);
		case NN_ELLIOT :
			value = fann_clip(value, 0.01f, 0.99f);
			return (fann_type) nn_elliot_derive(steepness, value, sum);
		case NN_ELLIOT_SYMMETRIC:
			value = fann_clip(value, -0.98f, 0.98f);
			return (fann_type) nn_elliot_symmetric_derive(steepness, value, sum);
		case NN_SIN_SYMMETRIC:
			return (fann_type) nn_sin_symmetric_derive(steepness, sum);
		case NN_COS_SYMMETRIC:
			return (fann_type) nn_cos_symmetric_derive(steepness, sum);
		case NN_SIN:
			return (fann_type) nn_sin_derive(steepness, sum);
		case NN_COS:
			return (fann_type) nn_cos_derive(steepness, sum);
		case NN_THRESHOLD:
			fann_error(NULL, FANN_E_CANT_TRAIN_ACTIVATION);
        case NN_BIHIPERBOLIC:
            return  nn_DerivateBiHiperbolic(value);
						  ...
						  fann_type value)
{
	...
	nn_activation_switch(activation_function, value, value);
	return value;
}
...
void fann_compute_MSE(struct fann *ann, fann_type * desired_output)
{
	...
	{
		ann->train_errors = (fann_type *) pgm_calloc(ann->total_neurons, sizeof(fann_type));
		if(ann->train_errors == NULL)
		{
...
void fann_update_weights(struct fann *ann)
{
	...
	if(ann->prev_weights_deltas == NULL)
	{
		ann->prev_weights_deltas =
			(fann_type *) pgm_calloc(ann->total_connections_allocated, sizeof(fann_type));
		if(ann->prev_weights_deltas == NULL)
		{
			fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
			return;
...
void fann_clear_train_arrays(struct fann *ann)
{
	...
	{
		ann->prev_steps = (fann_type *) pgm_malloc(ann->total_connections_allocated * sizeof(fann_type));
		if(ann->prev_steps == NULL)
		{
			fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
			return;
...
FANN_EXTERNAL enum nn_ActivationFuncEnum FANN_API
    fann_get_activation_function(struct fann *ann, int layer, int neuron)
{
	...
	if (neuron_it == NULL)
    {
		return (enum nn_ActivationFuncEnum)-1; /* layer or neuron out of bounds */
    }
    else
    {
