...
//Causaram problemas e não sao utilizadas!
/*
int ntohl( int n ){
    return n;
}

int htonl( int n ) { return n; }
*/

long *svd_longArray(long size, char empty, char *name) {
  long *a;
  if (empty) a = (long *) palloc0(size * sizeof(long));
  else a = (long *) palloc0(size * sizeof(long));
  if (!a) {
    perror(name);
    /* exit(errno); */
  }
...
double *svd_doubleArray(long size, char empty, char *name) {
  double *a;
  if (empty) a = (double *) palloc0(size * sizeof(double));
  else a = (double *) palloc0(size * sizeof(double));
  if (!a) {
    perror(name);
    /* exit(errno); */
  }
