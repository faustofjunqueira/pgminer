    ...
    Tree T;
{
	...
	if ( ! PossibleValues )
	{
	    PossibleValues = (Set *) pgm_calloc(MaxAtt+1, sizeof(Set));
	}
	...
	ForEach(a, 0, MaxAtt)
	{
	    if ( MaxAttVal[a] )
	    {
		PossibleValues[a] = (Set) pgm_malloc((MaxAttVal[a]>>3) + 1);
    ...
    Boolean UpdateTree;
{
    ...
    LocalClassDist = (ItemCount *) pgm_calloc(MaxClass+1, sizeof(ItemCount));
    ...
    Tree T;
{
	...
	HoldValues = (Set) pgm_malloc(Bytes);
