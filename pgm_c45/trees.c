...
/*************************************************************************/
/*									 */
/*	Display entire decision tree T in PrintedTRs			 */
/*									 */
/*************************************************************************/
    ...
    Tree T;
{
    ...
    PrintedTRs = (char*) pgm_malloc(1024 * 500);

    Subtree=0;
    sprintf(PrintedTRs, "Decision Tree:\n");
    sShow(T, 0);
    sprintf(PrintedTRs + strlen(PrintedTRs), "\n");
    ...
    short Sh;
{
	...
	if ( T != Nil && Sh && Sh * TabSize + MaxLine(T) > Width )
	{
	    ...
	    {
		Subdef[++Subtree] = T;
		sprintf(PrintedTRs + strlen(PrintedTRs), "[S%d]", Subtree);
	    }
	    else
	    {
		sprintf(PrintedTRs + strlen(PrintedTRs), "[S??]");
	    }
	}
	else
	{
    ...
    {
	sprintf(PrintedTRs + strlen(PrintedTRs)," %s (%.1f", ClassName[T->Leaf], T->Items);
	if ( T->Errors > 0 ) sprintf(PrintedTRs + strlen(PrintedTRs),"/%.1f", T->Errors);
	sprintf(PrintedTRs + strlen(PrintedTRs),")");
    }
}
    ...
    DiscrValue v;
{
	...
	case BrDiscr:
	    ...
	    sprintf(PrintedTRs + strlen(PrintedTRs),"%s = %s:", AttName[Att], AttValName[Att][v]);
	    break;
	...
	case ThreshContin:
	    ...
	    sprintf(PrintedTRs + strlen(PrintedTRs),"%s %s %g ",
		    AttName[Att], ( v == 1 ? "<=" : ">" ), T->Cut);

	    if ( T->Lower != T->Upper )
	    {
		sprintf(PrintedTRs + strlen(PrintedTRs),"[%g,%g]", T->Lower, T->Upper);
	    }
	...
	case BrSubset:
	    ...
	    if ( Values == 1 )
	    {
		sprintf(PrintedTRs + strlen(PrintedTRs),"%s = %s:", AttName[Att], AttValName[Att][Last]);
		break;
	    }

	   sprintf(PrintedTRs + strlen(PrintedTRs),"%s in {", AttName[Att]);
	    FirstValue = true;
	    Skip = TextWidth = strlen(AttName[Att]) + 5;
		...
		if ( In(Pv, T->Subset[v]) )
		{
		    ...
		    {
		  	sIndent(Sh, Tab);
			ForEach(i, 1, Skip) sprintf(PrintedTRs + strlen(PrintedTRs)," ");

			TextWidth = Skip;
			FirstValue = true;
		    }

		    sprintf(PrintedTRs + strlen(PrintedTRs),"%s%c", AttValName[Att][Pv], Pv == Last ? '}' : ',');
		    TextWidth += strlen(AttValName[Att][Pv]) + 1;
		    FirstValue = false;
		}
	    }
...
{
    sprintf(PrintedTRs + strlen(PrintedTRs), "\n");
    while ( Sh-- ) sprintf(PrintedTRs + strlen(PrintedTRs), "%s", Mark);
}
...
/**********************************************
/* PARTE QUE  SALVA e CARREGA ARVORE         */
/**********************************************
...
/*************************************************************************/
/*									 */
/*	Save entire decision tree T in string */
/*									 */
/*************************************************************************/
...

{
    ...
    TRs = (unsigned char*) pgm_malloc(1024*10);  // tamanho máximo do char* tree = 10kb
    ...
    TRsHEX = (char *) pgm_malloc(1024 *  10);
    char *p = TRsHEX;
    size_t offset;

    for(i = 0; i<indexTree; i++){
        offset = sprintf(p, "%02X",TRs[i]);
        p+=offset;
    }
    //elog(INFO,"MaxAtt %d", MaxAttVal[0]);
}
...
/*************************************************************************/
/*									 */
/*	Save tree T as characters (on string)				 */
/*									 */
/*************************************************************************/
    ...
    Tree T;
{
    ...
    sStreamOut((char *) &T->NodeType, sizeof(short));
    sStreamOut((char *) &T->Leaf, sizeof(ClassNo));
    sStreamOut((char *) &T->Items, sizeof(ItemCount));
    sStreamOut((char *) &T->Errors, sizeof(ItemCount));
    sStreamOut((char *) T->ClassDist, (MaxClass + 1) * sizeof(ItemCount));

    if ( T->NodeType )
    {
	sStreamOut((char *) &T->Tested, sizeof(Attribute));


	sStreamOut((char *) &T->Forks, sizeof(short));
	...
	switch ( T->NodeType )
	{
	    ...
	    case ThreshContin:
		sStreamOut((char *) &T->Cut, sizeof(float));
		sStreamOut((char *) &T->Lower, sizeof(float));
		sStreamOut((char *) &T->Upper, sizeof(float));
		break;
		...
		ForEach(v, 1, T->Forks)
		{
		    sStreamOut((char *) T->Subset[v], Bytes);
		}
		break;
	}
...
/*************************************************************************/
/*									 */
/*	Retrieve tree from saved characters in TRs				 */
/*									 */
/*************************************************************************/
...
/*   -------  */
{
    ...
    T = (Tree) pgm_malloc(sizeof(TreeRec));
    ...
    T->ClassDist = (ItemCount *) pgm_calloc(MaxClass+1, sizeof(ItemCount));
	...
	switch ( T->NodeType )
	{
	    ...
	    case BrSubset:
		T->Subset = (Set *) pgm_calloc(T->Forks + 1, sizeof(Set));

		Bytes = (MaxAttVal[T->Tested]>>3) + 1;
		ForEach(v, 1, T->Forks)
		{
		    T->Subset[v] = (Set) pgm_malloc(Bytes);
...
/*************************************************************************/
/*									 */
/*	Retrieve entire decision tree from unsigned char* binary TRs	 */
/*									 */
/*************************************************************************/
...
/*   --------  */
{
    ...
    int len  = strlen(TRsHEX);
    for(i = 0; i < len/2; i++){
	    substr[0] = TRsHEX[i*2];
	    substr[1] = TRsHEX[i*2 + 1];
	    sscanf( substr, "%hx", &TRs[i] );
    }
...
/*   -------  */
{
    ...
    sStreamIn((char *) &T->NodeType, sizeof(short));
    sStreamIn((char *) &T->Leaf, sizeof(ClassNo));
    sStreamIn((char *) &T->Items, sizeof(ItemCount));
    sStreamIn((char *) &T->Errors, sizeof(ItemCount));

    T->ClassDist = (ItemCount *) pgm_calloc(MaxClass+1, sizeof(ItemCount));
    sStreamIn((char *) T->ClassDist, (MaxClass + 1) * sizeof(ItemCount));

    if ( T->NodeType )
    {
	sStreamIn((char *) &T->Tested, sizeof(Attribute));
	sStreamIn((char *) &T->Forks, sizeof(short));
	...
	switch ( T->NodeType )
	{
	    ...
	    case ThreshContin:
		sStreamIn((char *) &T->Cut, sizeof(float));
		sStreamIn((char *) &T->Lower, sizeof(float));
		sStreamIn((char *) &T->Upper, sizeof(float));
		break;
		...
		{
		    T->Subset[v] = (Set) pgm_malloc(Bytes);
		    sStreamIn((char *) T->Subset[v], Bytes);
		}
	}

	T->Branch = (Tree *) pgm_calloc(T->Forks + 1, sizeof(Tree));
	ForEach(v, 1, T->Forks)
	{
	    T->Branch[v] = sInTree();
	}
...
/*************************************************************************/
/*									 */
/*	Stream characters to/from char* from/to an address		 */
/*									 */
/*************************************************************************/
...
/*  ---------  */
    ...
    int n;
{
    unsigned char *stemp = (unsigned char*) pgm_malloc(n);
    memcpy(stemp, s, n);
    memcpy(TRs + indexTree, stemp, n);
    indexTree += n;
}
    ...
    Tree T;
{
    ...
    New = (Tree) pgm_malloc(sizeof(TreeRec));
...
/*  -----------------  */
{
	...
	sStreamOut((char *) &MaxAttVal[Att], sizeof(int));

	ForEach(v, 1, MaxAttVal[Att])
	{
	    Length = strlen(AttValName[Att][v]) + 1;

	    sStreamOut((char *) &Length, sizeof(int));
	    sStreamOut((char *) AttValName[Att][v], Length);
	}
    }
}
...
/*  --------------------  */
{
	...
	sStreamIn(&MaxAttVal[Att], sizeof(int));

        ForEach(v, 1, MaxAttVal[Att])
        {
            sStreamIn(&Length, sizeof(int));

            AttValName[Att][v] = (char *) pgm_malloc(Length);
            sStreamIn(AttValName[Att][v], Length);
        }
    }
}
